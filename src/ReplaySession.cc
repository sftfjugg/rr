...
void ReplaySession::serialize_checkpoint(CheckpointInfo& cp_info) {
  DEBUG_ASSERT(clone_completion != nullptr);
  capnp::MallocMessageBuilder message;
  ScopedFd fd = cp_info.open_for_write();

  auto cc = message.initRoot<pcp::CloneCompletionInfo>();
  ...
  auto step =
      capnp::Data::Reader{ (std::uint8_t*)&current_step, sizeof(ReplayTraceStep) };
...
void ReplaySession::load_checkpoint(
    const CheckpointInfo& cp_info) {
  ...
  for (const auto& as : addr_spaces) {
    ...
    // calls
    {
      AutoRemoteSyscalls remote(leader, AutoRemoteSyscalls::DISABLE_MEMORY_PARAMS);
      leader->vm()->unmap_all_but_rr_mappings(remote);
      DEBUG_ASSERT(mappings_it->getMapType().isPrivateAnon() && (mappings_it->getProtection() & (PROT_READ | PROT_WRITE)) == (PROT_READ|PROT_WRITE));
      ...
      restore_map_contents(leader, data_to_str(mappings_it->getMapType().getPrivateAnon().getContentsPath()),
          stack_mapping);
      mappings_it++;
    }

    auto scratchPointer = remote_ptr<void>(cleader_captured_state.getScratchPtr());
