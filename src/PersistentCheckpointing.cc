...
void
write_vm(Task* clone_leader, rr::pcp::ProcessSpace::Builder builder,
  ...
  builder.setBreakpointFaultAddress(clone_leader->vm()->do_breakpoint_fault_addr().register_value());
  auto idx = 0;
...
void restore_map_contents(ReplayTask* t, const std::string& path, const KernelMapping& km) {
...
void update_persistent_checkpoint_index(
  ...
  auto checkpoints_index = trace_dir + "/checkpoints";
  ...
  for (auto& cp : cps) {
    auto find = std::find_if(checkpoints.cbegin(), checkpoints.cend(),
                             [&](auto& cp_) { return cp == cp_; });
    if (find == std::cend(checkpoints)) {
      cp.delete_from_disk();
    }
  }

  // remove old file
  remove(checkpoints_index.c_str());
  ...
  // and write a new one
  auto fd =
      ScopedFd(checkpoints_index.c_str(), O_EXCL | O_CREAT | O_RDWR, 0700);
  if (!fd.is_open())
    FATAL() << "failed to open file " << checkpoints_index;

  capnp::MallocMessageBuilder message;
  auto cc = message.initRoot<pcp::PersistentCheckpoints>();
  auto list = cc.initCheckpoints(checkpoints.size());
  auto idx = 0;
  for (const auto& cp : checkpoints) {
    auto cp_entry = list[idx++];
    cp_entry.setCloneCompletionFile(str_to_data(cp.capnp_cp_file));
    cp_entry.setId(cp.unique_id);
    auto tuid = cp_entry.initLastContinueTuid();
    tuid.setTid(cp.last_continue_tuid.tid());
    tuid.setSerial(cp.last_continue_tuid.serial());
    cp_entry.setWhere(str_to_data(cp.where));
    cp_entry.setNextSerial(cp.next_serial);
    auto stats = cp_entry.getStatistics();
    stats.setBytesWritten(cp.stats.bytes_written);
    stats.setSyscallsPerformed(cp.stats.syscalls_performed);
    stats.setTicksProcessed(cp.stats.ticks_processed);
    ...
    if (cp.is_explicit()) {
      ...
      mark_data_serializer(cp.clone_data, explicit_builder);
    } else {
      ...
      mark_data_serializer(cp.clone_data, mark_with_clone);
      ...
      mark_data_serializer(*cp.non_explicit_mark_data,
                           mark_with_gdb_checkpoint);
    }
  }
