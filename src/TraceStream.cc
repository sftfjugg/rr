...
void TraceReader::forward_to(FrameTime next_event_to_start_consuming) {
  ...
  while (!at_end()) {
    ...
    // means the EVENTS stream is at the correct time, now RAW_DATA and MMAPS must catch up 1 "step"
    if (frame.time() == stop_at) {
      auto& mmaps = reader(MMAPS);
      auto mmaps_pos_found = false;
      while(!mmaps.at_end() && !mmaps_pos_found) {
        // save state, if we find the MMAP record _after_ the one we're looking for
        // we need to restore it to this point.
        mmaps.save_state();
        CompressedReaderInputStream stream(mmaps);
        PackedMessageReader map_msg(stream);
        trace::MMap::Reader map = map_msg.getRoot<trace::MMap>();
      ...
      while (true) {
        ...
        bool found;
        KernelMapping km = read_mapped_region(&data, &found, TraceReader::DONT_VALIDATE, TimeConstraint::CURRENT_TIME_ONLY);
        if (!found) {
          break;
        }
      }
  ...
  FATAL() << "Could not forward stream(s) to event " << next_event_to_start_consuming;
}
